...
void channel_send_msg(char *channel, char *display_name, User *original_sender,
  ...
  for (u64 i = 0; i < users.size; i++) {
    ...
    if (user->user_state == STATE_OPEN && string_eq(user->channel, channel)) {
  ...
  case STATE_AUTH:
    if (message.message_type == MESSAGE_AUTH) {
      if (user_already_logged_in(message.username)) {
        send_reply(user, message, false, "This username is already logged in");
      } else {
...
void delete_users_in_end_state(void) {
  for (u64 i = 0; i < users.size; i++) {
    ...
    if (user->user_state == STATE_END && user->unconfirmed_messages.size == 0) {
      delete_user(user);
    }
  }
}
...
i32 get_timeout(void) {
  ...
  for (u64 user_idx = 0; user_idx < users.size; user_idx++) {
    ...
    for (u64 unconfirmed_idx = 0;
         unconfirmed_idx < user.unconfirmed_messages.size; unconfirmed_idx++) {
      i64 message_age =
          now - user.unconfirmed_messages.content[unconfirmed_idx].send_time_ms;
